...
Target auth model note:

- Readiness auth/portal phải được đọc theo mô hình `accountType + role + permission + scope`.
- `accountType` quyết định portal login và route guard.
- `role + permission + scope` quyết định action guard và data access guard.
- Runtime hiện tại vẫn còn mismatch vì login chuẩn trong code chủ yếu dựa trên `email/password`.

## Checklist chức năng + trạng thái hiện tại

| Chức năng yêu cầu                         | Tiêu chí kiểm tra                                                                                      | Trạng thái hiện tại                                      | Evidence chính                                                                                                                                                                                                                               |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Đăng nhập/đăng xuất                       | Login theo `accountType`, logout, thông báo lỗi đúng, điều hướng đúng portal                           | ✅ Đạt (target docs) / ⚠️ Một phần (runtime auth)        | `frontend/src/pages/LoginPage.tsx`, `backend/src/modules/auth/auth.routes.ts`, `docs/07-api-contract.md`, `docs/08-system-design.md`                                                                                                         |
...
| RBAC route frontend                       | Route guard theo `accountType`; action guard theo `role + permission + scope`                          | ✅ Đạt (target docs) / ⚠️ Một phần (runtime route split) | `frontend/src/App.tsx`, `frontend/src/auth/roles.ts`, `docs/08-system-design.md`                                                                                                                                                             |
...
| Dashboard theo vai trò                    | Dữ liệu theo `role + scope`, nhãn tiếng Việt dễ hiểu                                                   | ✅ Đạt                                                   | `frontend/src/pages/AdminDashboardPage.tsx`, `backend/src/modules/dashboard/dashboard.routes.ts`                                                                                                                                             |
