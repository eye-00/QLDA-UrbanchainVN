...
| QA/docs closeout Sprint 4 | Partial | Có full backend/frontend local gate pass | Chờ merge PR #28 (`docs-check`) | [backend/test/sprint3-registration.test.ts](../backend/test/sprint3-registration.test.ts), [backend/test/sprint4-service-wallet-governance.test.ts](../backend/test/sprint4-service-wallet-governance.test.ts), [docs/07-api-contract.md](./07-api-contract.md), [docs/08-definition-of-done.md](./08-definition-of-done.md) |

> Kết luận hiện tại: Sprint 4 đang ở trạng thái `Partial`; nâng `Done` sau khi PR #28 merge vào `develop` và required checks pass theo policy.
