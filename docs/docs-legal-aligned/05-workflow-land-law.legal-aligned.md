...
- Actor và routing theo mô hình chính quyền địa phương 2 cấp (NĐ 151/2025).
- Phí/lệ phí tiếp nhận hồ sơ và nghĩa vụ tài chính phải tách rõ ở off-chain.
- Bản đồ/hồ sơ địa chính bám chuẩn VN-2000 và quy định dữ liệu bản đồ địa chính (NĐ 101/2024).
- Chỉ ghi blockchain sau bước cập nhật hồ sơ địa chính/CSDL đất đai hợp lệ.
...
### 2.4. Nguyên tắc đăng nhập và phân hệ truy cập

- `accountType` dùng để xác định loại tài khoản đăng nhập và phân hệ được phép truy cập.
- `role` dùng để xác định vai trò nghiệp vụ trong quy trình xử lý.
- `permission` dùng để xác định thao tác cụ thể.
- `scope` dùng để giới hạn phạm vi cơ quan, địa bàn, phòng ban hoặc ownership.

Portal boundary chuẩn:

- `accountType = CITIZEN` truy cập `Portal người dân`.
- `accountType = STAFF` truy cập `Portal cán bộ`.
- `accountType = AGENCY_ADMIN` truy cập `Portal quản trị cơ quan`.
- `accountType = SYSTEM_ADMIN` truy cập `Portal quản trị hệ thống`.

Login identifier chuẩn:

- `CITIZEN`: đăng nhập bằng `citizenId` và mật khẩu.
- `STAFF`: đăng nhập bằng `officialUsername` hoặc `staffCode` và mật khẩu.
- `AGENCY_ADMIN`: đăng nhập bằng `username` quản trị cơ quan và mật khẩu.
- `SYSTEM_ADMIN`: đăng nhập bằng `username` quản trị hệ thống và mật khẩu.

Quy tắc:

- Người dân và doanh nghiệp/external applicant không đi qua `Portal cán bộ`.
- Cán bộ không thao tác công vụ trong `Portal người dân`.
- Quản trị cơ quan không dùng chức năng chỉ dành cho `SYSTEM_ADMIN`.
...
1. Người dân hoặc doanh nghiệp/external applicant đăng nhập vào `Portal người dân` bằng `citizenId` và mật khẩu, sau đó nộp hồ sơ điện tử.
...
1. Người chuyển nhượng/bên nhận đăng nhập đúng `Portal người dân`, sau đó tạo và nộp hồ sơ biến động.
