...
function toLandItem(item: {
  ...
  return {
    ...
    owner: item.owner
      ? { userId: item.owner.id, fullName: item.owner.fullName, email: item.owner.email }
      : null,
...
landRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    return ok(res, { query: keyword ?? null, items: items.map((item) => toLandItem(item)), total: items.length });
  })
);
    ...
    try {
      const land = await prisma.landParcel.create({
        data: {
          ...
          latitude: parsed.data.latitude !== undefined ? new Prisma.Decimal(parsed.data.latitude) : null,
          longitude: parsed.data.longitude !== undefined ? new Prisma.Decimal(parsed.data.longitude) : null
        },
        include: { owner: true }
      });
    ...
    try {
      const land = await prisma.landParcel.update({
        ...
        data: {
          ...
          ...(parsed.data.area !== undefined ? { area: new Prisma.Decimal(parsed.data.area) } : {}),
          ...(parsed.data.landUsePurpose !== undefined ? { landUsePurpose: parsed.data.landUsePurpose } : {}),
          ...(parsed.data.address !== undefined ? { address: parsed.data.address } : {}),
          ...(parsed.data.ownerUserId !== undefined ? { ownerUserId: parsed.data.ownerUserId } : {}),
          ...(parsed.data.latitude !== undefined ? { latitude: new Prisma.Decimal(parsed.data.latitude) } : {}),
          ...(parsed.data.longitude !== undefined ? { longitude: new Prisma.Decimal(parsed.data.longitude) } : {})
        },
        include: { owner: true }
      });
