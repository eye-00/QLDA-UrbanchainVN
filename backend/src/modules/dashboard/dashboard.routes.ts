  ...
  asyncHandler(async (req, res) => {
    ...
    if (user.role === "ADMIN") {
      const [
        ...
        prisma.registration.count({ where: { status: { in: ["CHO_TIEP_NHAN", "DA_TIEP_NHAN", "CAN_BO_SUNG"] } } })
      ]);
    ...
    if (user.role === "LAND_REGISTRY_OFFICER") {
      const [totalLands, appraisingQueue, waitingApproval, supplementQueue] = await Promise.all([
        prisma.landParcel.count(),
        prisma.registration.count({ where: { status: "DANG_THAM_DINH_VPDKDD" } }),
      ...
      return ok(res, {
        ...
        summary: {
          lands: { total: totalLands },
          queue: { appraising: appraisingQueue, waitingApproval, supplement: supplementQueue }
        }
      });
    }
    ...
    if (user.role === "RECEPTION_OFFICER") {
      ...
      return ok(res, {
        role: user.role,
        summary: {
          queue: { submitted: submittedQueue, supplement: supplementQueue, accepted: acceptedQueue }
        }
      });
    }
    ...
    return ok(res, {
      role: user.role,
      summary: {
        registrations: { total: myRegistrations, approved: myApprovedRegistrations },
