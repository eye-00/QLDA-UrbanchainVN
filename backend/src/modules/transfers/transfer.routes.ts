...
transferRouter.post("/", requireRoles(AUTH_ROLES.citizen), (req, res) => {
  ...
  if (!parsed.success)
    return badRequest(res, "Validation error", parsed.error.issues);
  const user = (req as AuthenticatedRequest).user;
...
transferRouter.post(
  ...
  (req, res) => {
    ...
    if (existing.fromUserId !== user.userId)
      return forbidden(res, "Bạn không có quyền xác nhận hồ sơ này");
...
transferRouter.post(
  ...
  (req, res) => {
    ...
    if (!result)
      return badRequest(
        res,
        "Không tìm thấy hồ sơ biến động hoặc thửa đất liên quan",
      );
